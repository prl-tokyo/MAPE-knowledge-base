...
sourceToSGList = $(update [p| S.Model {
...
sgToTuple = $(update [p| S.SecurityGroup {
                         S.sgID = id,
                         S.firewallRules = rules
                         }|]
                     [p| (id, rules) |]
                     [d| id = Replace; rules = Replace;|])
...
dupAndZipList :: (Show c, Show k, Eq k, Eq c) =>
                 c -> BiGUL [(k,[c])] [(k,c)]
dupAndZipList c0 = emb g p
  where
    ...
    get' bx s = case get bx s of Just v -> v
    put' bx s v = case put bx s v of Just s -> s
...
upFilterSep :: (Show c, Show k, Eq k, Eq c) =>
               c -> BiGUL (ListWithSep k c) [(k,c)]
upFilterSep c0 =
  Case [
    $(normalSV [p| [] |] [p| [] |] [p| [] |])
      ==> $(update [p| [] |] [p| [] |] [d| |]),
    ...
    $(normal [| \((k,c):s) v -> c==c0 |] [| \((k,c):s) -> c==c0 |])
      ==> $(update [p| _:v |] [p| v |] [d| v = upFilterSep c0 |]),

    $(adaptive [| \((k,c):s) v -> length v == 0 && c/=c0 |])
      ==> \(a:s) v -> s,

    $(normal [| \((k,c):s) ((k',c'):v) -> k==k' && c/=c0 |] [| \((k,c):s) -> c/=c0 |])
      ==> $(update [p| a:v |] [p| a:v |] [d| a = Replace; v = upFilterSep c0 |]),

    $(adaptive [| \((k,c):s) ((k',c'):v) -> k/=k' && c/=c0 |])
      ==> \(a:s) _ -> s
  ]

xs = [(100,1),(100,2),(100,3),(100,0),(200,5),(200,6),(200,0)]
xs2 = xs ++ xs
xs4 = xs2 ++ xs2
xs8 = xs4 ++ xs4

ys8 = [(100,[1,2,3]),(200,[5,6]),(100,[1,2,3]),(200,[5,6]),(100,[1,2,3]),(200,[5,6]),(100,[1,2,3]),(200,[5,6]),(100,[1,2,3]),(200,[5,6]),(100,[1,2,3]),(200,[5,6]),(100,[1,2,3]),(200,[5,6]),(100,[1,2,3]),(200,[5,6])]
...
upListWithGroup :: (Show c, Show k, Eq k, Eq c) =>
                   c -> BiGUL (ListWithSep k c) [(k,[c])]
upListWithGroup c0 =
  Case [
    $(normalSV [p| [] |] [p| [] |] [p| [] |])
    ...
    $(normal [| \(a:s) ((k,cs):v) -> fst a == k && length cs == 0 && snd a == c0 |]
             [| \(a:s) -> snd a == c0 |])
      ==> 
      $(rearrS [| \(a:s) -> (a,s) |]) $
         $(rearrV [| \((k,[]):v) -> ((k,()),v) |]) $
